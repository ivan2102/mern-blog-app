...
import { updateUserPassword } from "../../slices/authSlices";
...
export default function UpdatePassword() {
  ...
  const {user, isUpdatedPassword, appError, serverError} = resetPassword
  ...
  if(isUpdatedPassword) return <Navigate to={`/profile/${user?._id}`} />
  return (
    <div className="min-h-screen bg-gray-700  flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {/* Error */}
        <h3 className="text-red-500 text-center pt-2">{appError || serverError ? <p>{appError} - {serverError}</p> : null}</h3>
      </div>
